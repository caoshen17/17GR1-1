...
List<News> listNews=(List<News>)request.getAttribute("lis");
if(listNews==null){
	response.sendRedirect("util/news_control.jsp");
	return;
}
Page pa=(Page)request.getAttribute("pa");
    ...
    <div class="content">
      ...
      <ul class="classlist">
        ...
        for(News ne: listNews){
            %>
            <li><a herf="util/news_control.jsp"><%=ne.getNtitle()%></a><span><%=ne.getNcreatedate()%></span></li>
            <%
            i++;
            if(i%5==0){
        ...
        if(danqianye>1){
        	%>
          <a href="news_control.jsp?pageIndex=1">首页</a>&nbsp;
          <a href="news_control.jsp?pageIndex=<%=danqianye-1%>">上一页</a>
        	<% 
        }
        if(danqianye<zongyemian){
        	%>
          <a href="news_control.jsp?pageIndex=<%=danqianye+1%>">下一页</a>
          <a href="news_control.jsp?pageIndex=<%=zongyemian%>">末页</a> 
        <% 
        }
        %></p>
      </ul>
